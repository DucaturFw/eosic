...
exports.defaultProjectPath = path.resolve(__dirname, "..", "..", "default");
class EosDocker extends docker_1.default {
    defaultOptions() {
        return {
            ...
            logs(msg, ...args) {
                msg.split("\n").forEach((line) => {
                    console.log(chalk_1.default.gray(`[${this.options.container.name}]: ${line}`));
                });
            },
            errors(msg, ...args) {
                msg.split("\n").forEach((line) => {
                    console.log(chalk_1.default.red(`[${this.options.container.name}]: ${line}`));
                });
            }
        };
    }
