...
#   EOSIO_BIN_INSTALL_DIR="/usr/local"
# fi
EOSIO_BIN_INSTALL_DIR="/opt/eosio"
EOSIO_INSTALL_DIR=$EOSIO_BIN_INSTALL_DIR
...
#    echo mkdir $workdir/built
    ...
    for file in $@; do
        ...
        ($PRINT_CMDS; /opt/wasm/bin/clang -emit-llvm \
                                   -O3 --std=c++14 \
                                   --target=wasm32 -nostdinc \
    ...
    ($PRINT_CMDS; /opt/wasm/bin/llvm-link -only-needed -o $workdir/linked.bc $workdir/built/* \
                                   ${EOSIO_INSTALL_DIR}/usr/share/eosio/contractsdk/lib/libc.bc \
                                   ${EOSIO_INSTALL_DIR}/usr/share/eosio/contractsdk/lib/libc++.bc \
                                   ${EOSIO_INSTALL_DIR}/usr/share/eosio/contractsdk/lib/eosiolib.bc 
    )
    ($PRINT_CMDS; /opt/wasm/bin/llc -thread-model=single --asm-verbose=false -o $workdir/assembly.s $workdir/linked.bc)
